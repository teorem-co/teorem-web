...
const Navbar = () => {
  ...
  return (
    <div className='navbar'>
      ...
      <div>
        {user?.Role.abrv === RoleOptions.Tutor && (
          <div className='navbar__bottom navbar__bottom__share'
               onClick={shareProfile}>
            <div className='navbar__bottom__share-profile'>
              <div className='navbar__bottom__share--icon'>
                <i
                  className={`icon icon--base icon--grey ${textCopiedToClipboard ? 'icon--check' : 'icon--share'}`}></i>
              </div>
              <div className='navbar__bottom__user-info'>
                <div
                  className='type--color--tertiary type--wgt--bold type--break'>
        ...
        <div className='navbar__bottom'>
          ...
          {(user?.Role?.abrv === RoleOptions.Child && (
            <div className='flex flex--grow flex--center'>
              ...
              <div className='navbar__bottom__user-info'>
                <div
                  className='type--color--primary type--wgt--bold type--break'>
                  {user?.firstName} {user?.lastName}
                </div>
                <div
                  className='type--xs type--color--secondary type--wgt--regular '>{user?.Role?.name}</div>
              </div>
            </div>
          )) || (
            <NavLink to={PROFILE_PATHS.MY_PROFILE_INFO_PERSONAL}
                     className='navbar__bottom__my-profile'
                     activeClassName='active'>
              <div className='navbar__bottom__avatar pos--rel'>
                {user?.profileImage ? (
                  <img
                    src={profileImageURL ? `${profileImageURL}&v=${cacheBuster}` : gradientCircle}
                    alt='avatar' />
                ) : (
                  <ImageCircle
                    initials={`${user?.firstName.charAt(0)}${user?.lastName.charAt(0)}`} />
          ...
          <a href={`${landingHostName}${landingPath}`} onClick={handleLogout}
             className='d--ib'>
