...
import {
  TutorItemVideoPopup,
} from '../../../features/searchTutors/components/TutorItemVideoPopup';
    ...
    <>
      ...
      {showMessagePopup && (
        <div className={'video-modal__overlay'}>
          <div
            className='flex flex--col flex--jc--space-between p-10 background-red modal dash-border confirm-modal'>
            <h2
              className={'align-self-center'}>{tutorVideoInfo.firstName + ' ' + tutorVideoInfo.lastName}</h2>
            <p
              className={'align-self-center mt-2'}> {t('TUTOR_MANAGMENT.VIDEO_PREVIEW.MESSAGE_TITLE')}</p>
            <textarea
              value={message}
              name='message'
              id='id-message'
