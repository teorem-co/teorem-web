...
const UpdateBooking: React.FC<IProps> = (props) => {
  ...
  const [getUnavailableDays, {
    ...
    isLoading: unavailableDaysIsLoading,
  }] = useLazyGetTutorUnavailableDaysQuery();
...
// Convert to UTC and format
  ...
  async function fetchData() {
    if (booking?.tutorId)
      await getUnavailableDays(booking?.tutorId).unwrap().then();
  }
  ...
  useEffect(() => {
    if (booking) {
      ...
      if (userRole === RoleOptions.Tutor) {
        ...
        getStudentAvailablePeriods(params).unwrap().then(res => {
        ...
        getTutorAvailablePeriods(params).unwrap().then(res => {
          setAvailableDates(res);
          formik.setFieldValue(formik.getFieldProps('selectedTime').name, (moment(res[0]).format('HH:mm')));
        });
      }
        ...
        <div style={mobileStyles}
            ...
            <div className='mb-4'>
              {userRole !== RoleOptions.Tutor &&
                <div className='flex flex--center mb-4'>
                  <i
                    className='icon icon--base icon--tutor icon--grey mr-4'></i>
                  <div
                    className='type--color--secondary'>{booking?.User.firstName}</div>
                </div>}

              <div className='flex flex--center mb-4'>
                <i
                  className='icon icon--base icon--subject icon--grey mr-4'></i>
              ...
              {userRole === RoleOptions.Student ? (
                ...
                <div className='flex flex--center'>
                  <i
                    className='icon icon--base icon--child icon--grey mr-4'></i>
                  <div
                    className='type--color--secondary w--100'>{booking?.userFullName}</div>
                </div>
              )}
            </div>
            <FormikProvider value={formik}>
              <Form id='updateBookingForm'>
                <div className='field'>
                  ...
                  <div
                    className='flex flex--jc--space-between flex--row w--100 mr-6 mt-5'>

                    {
                      <>
                        <DatePicker
                          ...
                          onChange={(newValue) =>
                            formik.setFieldValue(formik.getFieldProps('selectedDate').name, newValue?.format('YYYY-MM-DD'))}

                        />

                        <FormControl className={'w--45'}>
                          <InputLabel
                            id='selectedTime'>{t('BOOK.FORM.TIME_OF_BOOKING')}</InputLabel>
                          <Select
                            ...
                            input={<OutlinedInput
                              label={t('BOOK.FORM.TIME_OF_BOOKING')} />}
                            MenuProps={MenuProps}
                          >
          ...
          <div className='modal--parent__footer'>
            ...
            <button
              ...
              disabled={moment(start, 'HH:mm').isSame(moment(formik.values.selectedTime, 'HH:mm'))}>
              {t('BOOK.FORM.UPDATE')}
            </button>
            <button
