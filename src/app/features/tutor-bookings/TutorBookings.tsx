...
const TutorBookings = () => {
  ...
  const slotSelect = (e: SlotInfo) => {
    ...
    if (firstCheck && !moment.utc(e.start).isBefore(moment().add(3, 'hours')) && isAvailableBooking) {
      // setSelectedStart(moment(e.start).format('DD MMM YYYY, HH:mm'));     MMMM format doesn't work with different languages!
  ...
  const handleSelectedEvent = (e: IBookingTransformed) => {
    ...
    if (e.userId === userId) {
      if (moment(e.start).isBefore(moment()) || emptyBookings.length > 0) {
        ...
        //     label: e.label,
        // });
        setSelectedStart(moment.utc(e.start).format('DD MMM YYYY, HH:mm'));
