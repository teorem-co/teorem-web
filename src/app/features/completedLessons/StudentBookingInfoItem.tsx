...
import { IMeetRecording, useLazyGetLessonRecordingsQuery } from '../../services/hiLinkService';
...
const StudentBookingInfoItem: React.FC<Props> = ({ bookingInfo, activeLesson }) => {
    ...
    return (
        <>
            <div key={bookingInfo.bookingId} className={`completed-lessons-list__item mt-3 mb-0`}>
                    ...
                    <div>
                        ...
                        <div className="type--wgt--light">
                            {`${t(`COMPLETED_LESSONS.DATE_TITLE`)}: ` +
                                ...
                                '.' +
                                moment(bookingInfo.startTime).toDate().getFullYear() +
                                ` ${t('COMPLETED_LESSONS.DATE_AT_WORD')} ` +
                ...
                {lessonRecordingModal ? (
                    <LessonRecordingModal activeRecording={activeRecording!} handleClose={() => setLessonRecordingModal(false)} />
                ) : (
                    <></>
                )}
            </div>
            {dropdownVisible && (
                <div className={`container-dropdown completed-lessons-list__item dropdown-content ${dropdownVisible ? 'show-dropdown' : ''}`}>
                    {isLoading ? <BeatLoader size={10} color="#7e6cf2" /> : null}
                    {recordings?.map((recording) => {
                        return (
                            <div className="p-3 lesson-row">
