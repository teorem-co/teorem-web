...
export const filterArrayUniqueMessages = (arr: Array<ISendChatMessage>) => {
    const set = new Set();
    return arr.filter((o) => {
        if (o.message.messageMissedCall) return true;
...
const chatSlice = createSlice({
    ...
    reducers: {
        ...
        setActiveChatRoom(state, action: PayloadAction<IChatRoom | null>) {
            if (action.payload) state.activeChatRoom = action.payload;
        },
        ...
        addChatRooms(state, action: PayloadAction<Array<IChatRoom> | null>) {
            if (action.payload) {
                ...
                for (let j = 0; j < action.payload.length; j++) {
                    ...
                    for (let i = 0; i < state.chatRooms.length; i++) {
                        if (
                            state.chatRooms[i].tutor?.userId == action.payload[j].tutor?.userId &&
                            state.chatRooms[i].user?.userId == action.payload[j].user?.userId
                        ) {
                    ...
                    if (!inside) state.chatRooms.push(action.payload[j]);

                    unreadMessages += action.payload[j].unreadMessageCount;
                }

                if (state.newMessages != null) state.newMessages += unreadMessages;
                else state.newMessages = unreadMessages;
            }
        },
                ...
                for (let i = 0; i < state.chatRooms.length; i++) {
                    if (state.chatRooms[i].tutor?.userId == action.payload.tutorId) {
                        ...
                        if (state.chatRooms[i].tutor?.userId == action.payload.senderId) {
                            ...
                            if (state.newMessages != null) state.newMessages += 1;
                            else state.newMessages = 1;
                            return;
                        }
                    }
                }
                ...
                for (let i = 0; i < state.chatRooms.length; i++) {
                    ...
                    for (let k = 0; k < action.payload.length; k++) {
                        if (state.chatRooms[i].tutor?.userId == action.payload[k].tutorId) {
                            ...
                            if (state.activeChatRoom) state.activeChatRoom.messages.push(action.payload[k]);
                    ...
                    if (state.activeChatRoom) state.activeChatRoom.messages = [...newMessages, ...state.activeChatRoom.messages];
                }
            }
        },
            ...
            for (let i = 0; i < state.chatRooms.length; i++) {
                if (state.chatRooms[i].user?.userId == action.payload.userId && state.chatRooms[i].tutor?.userId == action.payload.tutorId) {
                    ...
                    messages = messages.filter((message: ISendChatMessage) => {
                        if (!message.message.messageId) return true;
                    ...
                    if (state.activeChatRoom) state.activeChatRoom.messages = messages;
                }
            }
        },
        ...
        addMessage(state, action: PayloadAction<ISendChatMessage | null>) {
            if (action.payload) {
                for (let i = 0; i < state.chatRooms.length; i++) {
                    ...
                    for (let j = 0; j < state.chatRooms[i].messages.length; j++) {
                        if (
                            !action.payload.message.messageMissedCall &&
                            !action.payload.message.messageNew &&
                            state.chatRooms[i].messages[j].message.messageId == action.payload.message.messageId
                        ) {
                            return;
                        }
                    }
                    ...
                    if (state.chatRooms[i].tutor?.userId == action.payload.tutorId && state.chatRooms[i].user?.userId == action.payload.userId) {
                        ...
                        if (
                            state.chatRooms[i].tutor?.userId == state.activeChatRoom?.tutor?.userId &&
                            state.chatRooms[i].user?.userId == state.activeChatRoom?.user?.userId
                        ) {
                        ...
                        if (action.payload.message.messageMissedCall) {
                            ...
                            if (state.activeChatRoom) {
                                if (state.activeChatRoom.unreadMessageCount) state.activeChatRoom.unreadMessageCount += 1;
                                else state.activeChatRoom.unreadMessageCount = 1;
                            }
            ...
            if (action.payload) {
                for (let i = 0; i < state.chatRooms.length; i++) {
                    for (let j = 0; j < state.chatRooms[i].messages.length; j++) {
                        if (state.chatRooms[i].messages[j].message.messageId == action.payload.message.messageId) {
                            if (
                                (state.chatRooms[i].messages[j].message.messageNew && !state.chatRooms[i].messages[j].message.messageMissedCall) ||
                                state.chatRooms[i].messages[j].message.isRead
                            )
                                return;
                            ...
                            if (state.newMessages != null) {
                                state.newMessages -= 1;
                            } else state.newMessages = 0;
                            if (
                                !(state.chatRooms[i].messages[j].message.messageNew && !state.chatRooms[i].messages[j].message.messageMissedCall) &&
                                state.activeChatRoom?.tutor?.userId == state.chatRooms[i].messages[j].tutorId &&
                                state.activeChatRoom?.user?.userId == state.chatRooms[i].messages[j].userId
                            ) {
                                state.activeChatRoom.unreadMessageCount = Math.max(0, state.activeChatRoom.unreadMessageCount - 1);
                            }

                            if (state.chatRooms[i].messages[j].message.messageId) state.socket.emit('readMessage', state.chatRooms[i].messages[j]);
                            return;
                        }
                    }
                }
                ...
                for (let i = 0; i < state.chatRooms.length; i++) {
                    if (state.chatRooms[i].user?.userId == action.payload.userId && state.chatRooms[i].tutor?.userId == action.payload.tutorId) {
                        ...
                        for (let j = 0; j < state.chatRooms[i].messages.length; j++) {
                            ...
                            if (state.user?.userId !== state.chatRooms[i].messages[j].senderId) state.chatRooms[i].messages[j].message.isRead = true; //TODO: check this because it sets that messages are read,
                        }
                    }
                }
                return state;
        ...
        addChatRoom(state, action: PayloadAction<IChatRoom | null>) {
            if (action.payload) {
                ...
                for (let i = 0; i < state.chatRooms.length; i++) {
                    if (
                        state.chatRooms[i].tutor?.userId == action.payload.tutor?.userId &&
                        state.chatRooms[i].user?.userId == action.payload.user?.userId
                    ) {
                        ...
                        for (let j = 0; j < action.payload.messages.length; j++) {
                            state.chatRooms[i].messages.find((x: ISendChatMessage) => {
                                ...
                                return (
                                    x.message.messageId == action.payload?.messages[j].message.messageId &&
                        ...
                        state.chatRooms[i].messages = state.chatRooms[i].messages
                            .concat(action.payload?.messages)
                            .sort((a: ISendChatMessage, b: ISendChatMessage) =>
                                new Date(a.message.createdAt) > new Date(b.message.createdAt) ? 1 : -1
                            );
                ...
                if (state.newMessages) {
                    if (action.payload.unreadMessageCount > 0) state.newMessages += action.payload.unreadMessageCount;
                } else {
