...
export const onboardingSlice = createSlice({
    ...
    reducers: {
        ...
        setStepTwo(state, action: PayloadAction<IStepTwo>) {
            const { currentOccupation, yearsOfExperience, aboutYou, aboutYourLessons } = action.payload;
        ...
        setStepThree(state, action: PayloadAction<IStepThree>) {
            const { address, addressLine2, postcode, city, country, IBAN } = action.payload;
...
export const { setStepZero, setStepOne, setStepTwo, setStepThree, setStepFour, resetOnboarding } = onboardingSlice.actions;
